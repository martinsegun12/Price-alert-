            ...
            try:
                ...
                with alert_lock:
                    if aid in alerts:
                        ...
                        alerts[aid]['market_open'] = is_market_open(pair)

                # Only trigger if market is open
                if abs(price - target) <= tol and is_market_open(pair):
                    with alert_lock:
                        if aid in alerts:
                    ...
                    threading.Thread(
                        ...
                        daemon=True).start()
                elif abs(price - target) <= tol and not is_market_open(pair):
                    add_log(f'⏸ {pair} near POI but market is CLOSED — waiting for open')
...
def is_market_open(pair):
    """
    Forex market: Sun 10pm - Fri 10pm Nigeria time (UTC+1)
    Indices: Mon-Fri with specific hours
    """
    now_utc = datetime.now(timezone.utc)
    now_ng = now_utc.hour + 1  # Nigeria is UTC+1
    weekday = now_utc.weekday()  # 0=Mon, 6=Sun

    upper = pair.upper().replace('/','')

    # Indices - weekdays only, roughly 8am-10pm Nigeria time
    indices = ['SPX','NAS100','US30','GER40','UK100','JP225','AUS200']
    if any(idx in upper for idx in indices):
        if weekday >= 5:  # Saturday or Sunday
            return False
        return 8 <= now_ng <= 22

    # Forex - Sun 10pm to Fri 10pm Nigeria time
    if weekday == 5:  # Saturday - always closed
        return False
    if weekday == 6:  # Sunday - open after 10pm Nigeria (9pm UTC)
        return now_utc.hour >= 21
    if weekday == 4:  # Friday - closed after 10pm Nigeria (9pm UTC)
        return now_utc.hour < 21
...
def market_status():
    ...
    with alert_lock:
        for aid, a in alerts.items():
            statuses[aid] = is_market_open(a['pair'])
    return jsonify({'statuses': statuses, 'server_time': datetime.now(timezone.utc).strftime('%H:%M UTC')})
